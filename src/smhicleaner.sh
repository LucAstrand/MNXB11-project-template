...
# - Convert format to commas instead of ; (sed)
# - Remove extra commas when necessary (sed)
log "Perform cleanup in one line, result in $CLEANER_BAREDATAFILENAME"
tail -n +$STARTLINE $CLEANER_ORIGINALFILENAME | cut -d';' -f 1,2,3,4,5 | sed 's/;/,/g' | sed 's/,$//' > $CLEANER_BAREDATAFILENAME


# Define your header row (modify as per the actual column names)
header="Datum, Tid (UTC), Lufttemperatur, Kvalitet" 

# Create a temporary file to store the header and cleaned data together
temp_file=$(mktemp)

# Write the header to the temporary file
echo "$header" > "$temp_file"

# Append the contents of the cleaned file to the temporary file
cat "$CLEANER_BAREDATAFILENAME" >> "$temp_file"

# Replace the original cleaned file with the temporary file (now with the header)
mv "$temp_file" "$CLEANER_BAREDATAFILENAME"

log "Added header to $CLEANER_BAREDATAFILENAME"
